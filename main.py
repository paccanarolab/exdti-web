...
def get_targets_for_drug(drug_name: str):
    ...
    ranked_targets = df_predictions[df_predictions["drug_name"] == drug_name] \
                        .sort_values("Predicted_Score", ascending=False)[["protein_name", "Predicted_Score"]]
...
def get_drugs_for_target(protein_name: str):
    ...
    ranked_drugs = df_predictions[df_predictions["protein_name"] == protein_name] \
...
def get_prediction_rank(drug_name: str, protein_name: str):
    ...
    drug_subset = df_predictions[df_predictions["drug_name"] == drug_name].sort_values("Predicted_Score", ascending=False)
    ...
    target_subset = df_predictions[df_predictions["protein_name"] == protein_name].sort_values("Predicted_Score", ascending=False)
...
def get_known_interactions(entity_type: str, entity_name: str):
    ...
    if entity_type == "drug":
        ...
        known_target_names = df_predictions[df_predictions["Target_Index"].isin(known_targets)]["protein_name"].unique().tolist()
        ...
        known_drug_names = df_predictions[df_predictions["Drug_Index"].isin(known_drugs)]["drug_name"].unique().tolist()
